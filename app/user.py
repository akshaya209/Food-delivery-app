...
def validate_user(user: dict) -> bool:
    """
    Validate a user dict has required fields: name and address.
    Returns True if valid, False otherwise.
    """
    required = {"name", "address"}
    return required.issubset(user.keys())


def get_notifications(user_id: str) -> list:
    """Return all notifications stored for a user."""
    db = get_db_connection()
    return db.get("notifications", {}).get(user_id, [])
